...
testDataset = np.loadtxt("%s/DUMP_out.txt"%RnnTest_path, delimiter=",")
...
for label, hour, minute in zip(yo, test_hour,test_min):
    if label == 1:
        print("\n Test time (hour:minute) %d : %d => An anomaly is detected!" % (hour, minute))
    else:
        print("\n Test time (hour:minute) %d : %d => Normal traffic" % (hour, minute))
end = time.clock()
