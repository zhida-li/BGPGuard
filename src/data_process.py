...
def normTrainTest(cut_pct, site):
    train = np.loadtxt('./src/data_split/train_%s_%s.csv' % (cut_pct, site), delimiter=',')
    test = np.loadtxt('./src/data_split/test_%s_%s.csv' % (cut_pct, site), delimiter=',')
    ...
    np.savetxt('./src/data_split/train_%s_%s_n.csv' % (cut_pct, site), train_n, delimiter=',')  # ,fmt='%.4f')
    np.savetxt('./src/data_split/test_%s_%s_n.csv' % (cut_pct, site), test_n, delimiter=',')  # ,fmt='%.4f')
